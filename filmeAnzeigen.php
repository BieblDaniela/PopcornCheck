...
try {
    $stmt2 = $pdo->prepare('SELECT bewertung.bid, bewertung.bewertung, bewertung.sterne, konto.email FROM bewertung INNER JOIN konto ON bewertung.kid_fk = konto.kid WHERE bewertung.fid_fk = :fid order by bid desc');
    ...
    <form action="" method="post">
        ...
        <div class="movie-details-layout">
            ...
            <div class="detail-info-pane">
                <div style="margin-bottom: 24px;">
                    <span class="badge badge-genre"
                        style="margin-bottom: 8px;"><?= htmlspecialchars($film['genre']) ?></span>
                    <h1 style="font-size: 2.2rem; margin-bottom: 12px;"><?= htmlspecialchars($film['titel']) ?></h1>
                </div>

                <?php if (!empty($film['trailer'])): ?>
                    <div class="trailer-container">
                        <iframe src="https://www.youtube.com/embed/<?= htmlspecialchars($film['trailer']) ?>"
                ...
                <div class="detail-meta-item">
                    ...
                    <p style="white-space: pre-line; line-height: 1.7;"><?= htmlspecialchars($film['beschreibung']) ?>
                    </p>
                </div>
            ...
            <div class="detail-reviews-pane">
                <h2
                    style="font-size: 1.4rem; margin-bottom: 20px; border-bottom: 1px solid var(--bg-card-border); padding-bottom: 12px;">
                    Community Bewertungen</h2>

                <div class="table-container"
                    style="margin: 0; background: transparent; border: none; box-shadow: none;">
                            ...
                            <?php if (!empty($bewertungen)): ?>
                                <?php foreach ($bewertungen as $b): ?>
                                    <tr>
                                        <td>
                                            <span
                                                style="font-weight: 500; color: #ffffff;"><?= htmlspecialchars($b['email']) ?></span>
                                        </td>
                                        <td style="text-align: right; font-weight: 700; color: var(--primary-color);">
                                            ...
                                            <br>
                                            <span
                                                style="font-size: 0.8em; font-weight: normal; color: #ccc;"><?= htmlspecialchars($b['bewertung']) ?></span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="2"
                                        style="text-align: center; font-style: italic; color: var(--text-muted); padding: 40px 0;">
